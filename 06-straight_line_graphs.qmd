...
```{r, echo=FALSE, eval=(knitr::pandoc_to("html"))}
knitr::include_url("https://www.desmos.com/calculator/jzyasdusre?embed", height="500px")
```
...
```{r, echo=FALSE, eval=(knitr::pandoc_to("html"))}
knitr::include_url("https://numbas.mathcentre.ac.uk/question/68532/linear-graphs-plotting-1-integer-values/embed/?token=c4963f30-f814-4d6f-91bc-b631f0ed1ec3", height="400px")
```

:::{.callout-tip}
## Curves and straight lines

In mathematics, a straight line is technically a special type of curve, just one with no curvature at all. You will often hear mathematicians refer to any line, straight or otherwise, as a "curve", particularly when its exact shape has not yet been determined.
:::
...
Once we've done this we can see that the intercept is when $y=-2$. Notice if we substituted $x=0$ in the original equation we would get this answer too.
...
:::{.callout-note}
* $m$ is the gradient - the amount $y$ changes for an increase in $1$ in the $x$ direction
...
P = 20 - 2Q
$$
where $P$ is the price and $Q$ the quantity.

The intercept $20$ is the maximum price anyone would pay (when $Q=0$). The gradient $-2$ means that price must fall by $2$ for each additional unit demanded.

This responsiveness is an example of what economists call **elasticity**. Here, because we are looking at how quantity demanded responds to a change in price, the specific name is **price elasticity of demand**. The gradient of the demand curve gives a rough guide: a steep curve means buyers are not very responsive, so demand is **inelastic**; a flatter curve means buyers are very responsive, so demand is **elastic**.

```{r, echo=FALSE, fig.height=7, fig.width=5, fig.align='center', fig.alt='A downward-sloping straight line showing a demand curve with intercept 20 on the price axis and slope negative 2. Dashed blue lines show a horizontal change of 5 and a vertical change of negative 10 between two points on the line.'}
Q <- seq(0, 12, length.out = 200)
P <- 20 - 2*Q
plot(Q, P, type = "n", xlim = c(0, 12), ylim = c(0, 24),
     xlab = "Quantity, Q", ylab = "Price, P",
     main = expression(P == 20 - 2*Q),
     xaxp = c(0, 10, 5), yaxp = c(0, 20, 4),
     panel.first = grid())
lines(Q, P, lwd = 2)
points(0, 20, col = "blue", pch = 19, cex = 1.3)
points(5, 10, col = "blue", pch = 19, cex = 1.3)
segments(0, 20, 5, 20, col = "blue", lty = 2, lwd = 1.5)
segments(5, 20, 5, 10, col = "blue", lty = 2, lwd = 1.5)
text(2.5, 21.5, "5", col = "black", cex = 1.1)
text(6.2, 15, "-10", col = "black", cex = 1.1)
```

To calculate the gradient we divide the change in the $y$ direction by the change in the $x$ direction. Here this is $\frac{-10}{5} = -2$.

Because the curve slopes downward the gradient is negative, and economists write $\varepsilon = -2$. Much to the annoyance of mathematicians, they will often drop the minus sign and simply give the positive value, particularly when it is obvious that the gradient is negative.
...
```{r, echo=FALSE, eval=(knitr::pandoc_to("html"))}
knitr::include_url("https://numbas.mathcentre.ac.uk/question/104226/linear-graphs-reading-gradient-and-intercept-from-an-equation/embed/?token=2d71329a-7508-4dfa-9115-96abe822746a", height="400px")
```
