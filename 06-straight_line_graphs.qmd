...
```{r chunk-set, include=FALSE}
knitr::opts_chunk$set(
  screenshot.opts = list(file = 'img.png', cliprect = 'viewport', delay = 2)
)
```
...
$$
\begin{array} {|r|r|}\hline x & y \\ \hline 0 & \frac{1}{2}(0)+1=1 \\
\hline 1 & \frac{1}{2}(1)+1=1.5 \\
\hline 2 & \frac{1}{2}(2)+1=2 \\ \hline  \end{array}
$$

Once we have these values they can be plotted on graph.
```{r desmos-img-1, echo=FALSE}
...
**Pro tip** A change in a quantity is often represented by the Greek letter delta, $\Delta$, so we can rewrite $m$ as: $m = \frac{\Delta y}{\Delta x}$
...
Using you knowledge of $y=mx+c$ try the following questions. Don't be afraid to look at the answers and then try a fresh set of questions if it seems tricky at first.
