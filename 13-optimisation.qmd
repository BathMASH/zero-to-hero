...
knitr::include_url("https://www.desmos.com/calculator/xknnmfqryk?embed", height="500px")
```
...
We can classify the turning points by looking at the values of the first and second derivatives, $f'(x)$ and $f''(x)$. The graph below shows $f(x)$, $f'(x)$ and $f''(x)$ all plotted on the same axes. As you move the point notice how when $f(x)$ is at it's maximum,  $f'(x) = 0$ and $f''(x) < 0$. The three different lines are as follows: $f(x)$ is solid red, $f'(x)$ is dashed blue and $f''(x)$ is dotty and green. There is a lot going on in the graph below but it's worth taking some time to play with the points to see what is going on.

```{r, echo=FALSE, eval=(knitr::pandoc_to("html"))}
knitr::include_url("https://www.desmos.com/calculator/ybaguwnu8g?embed", height="500px")
```
...
Have a go at this question on classifying critical points.
...
Using the equations for $P$, $MC$ and $FC$ given in this question we can substitute these in our equation for profit to give:
...
So finally the quantity $q$ that maximises profits is $6$.
