...
A firm's revenue is given by $R = (100 - 2q)^3$, where $q$ is quantity. To find the marginal revenue we use the chain rule. Let $u = 100 - 2q$, so $R = u^3$. Then $\frac{dR}{dq} = \frac{dR}{du} \times \frac{du}{dq} = 3u^2 \times (-2) = -6(100-2q)^2$.
::::
