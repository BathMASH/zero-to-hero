...
```{r, echo=FALSE}
knitr::include_url("https://www.desmos.com/calculator/u1nuc4eumz?embed", height="500px")
```
...
```{r, echo=FALSE}
knitr::include_url("https://www.desmos.com/calculator/d635xag3xe?embed", height="500px")
```
...
If a sum of money grows continuously at an annual rate of $5\%$, its value after $t$ years is:

$$
V(t) = V_0 e^{0.05t}
$$

For example, £100 invested today will be worth:

$$
V(10) = 100 \times e^{0.05 \times 10} = 100 \times e^{0.5}
$$

which is approximately $164.87.
$$

after ten years.

```{r, echo=FALSE, fig.height=7, fig.width=5, fig.align='center', fig.alt='A graph showing continuous exponential growth of an investment starting at 100 pounds and growing at 5 percent per year. After 10 years the value is approximately 165 pounds.'}
t <- seq(0, 20, length.out = 200)
V <- 100 * exp(0.05 * t)
plot(t, V, type = "n", xlim = c(0, 20), ylim = c(0, 300),
     xlab = "Time (years)", ylab = "Value, V",
     main = expression(V(t) == 100*e^{0.05*t}),
     xaxp = c(0, 20, 4), yaxp = c(0, 300, 5),
     axes = FALSE)
abline(h = seq(0, 300, by = 60), v = seq(0, 20, by = 5), col = "grey90", lty = "dotted")
axis(1, at = seq(0, 20, by = 5))
axis(2, at = seq(0, 300, by = 60), las = 1)
box()
lines(t, V, lwd = 2)
points(10, 100*exp(0.5), col = "blue", pch = 19, cex = 1.3)
```
::::
...
```{r, echo=FALSE}
knitr::include_url("https://numbas.mathcentre.ac.uk/question/62791/differentiation-using-a-table-of-derivatives-4/embed/?token=c0e0c050-43ee-44bf-ac18-9377c9d3a522", height="650px")
```
