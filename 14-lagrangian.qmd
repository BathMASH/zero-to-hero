...
This is a quick start guide to a method of optimisation used for when you have multiple variables and another function that constrains them. I'm not going to prove anything here and we'll only cover situations with two variables. It's also the most complex thing in this guide.
...
Here are other notation for taking the determinant. For a matrix $A$.
$$
\boldsymbol{A}=\begin{pmatrix}
a & b \\
c & d
...
```{r, echo=FALSE, eval=(knitr::pandoc_to("html"))}
knitr::include_url("https://numbas.mathcentre.ac.uk/question/175721/determinant-of-a-2x2-matrix/embed/", height="400px")
```
...
Now we have the tools we need lets look at an example. In a nutshell the method of using a lagrangian multiplier collects all the equations we need to solve a constrained optimisation problem in one place. Optimisation means, find the biggest or smallest values and constrained means we are limiting the values to try. Under the hood the method finds the points where the gradient of the function we want to optimise is parallel to the gradient of the constraint. This is where the Greek letter lambda $\lambda$ comes from it allows gradients that are parallel but different magnitudes to be equal to one another.

### Method

If we have a function that we want to optimise, say $f(x,y)$, that is subject to the constraint $g(x,y)=k$ where $k$ is a constant. The we create the equation:
...
We then find the partial derivatives of $L$ with respect to each variable $x$, $y$ and $\lambda$. Set them equal to zero (just like regular optimisation), then solve this system of equations.

We then look at the determinant of the bordered hessian (this is a special second derivative test for Lagrangian Multipliers and much like the second derivative test in normal optimisation). The bordered hessian is:
...
If $B > 0$ we have a local maximum.

If $B < 0$ we have a local minimum.
...
Omitting the variables and differentiating with respect to each variable we have.
...
Substituting $(4)$, $x^2 = 4y^2$, into $x^2 +2y^2 = 6$ we get:
...
Next take the case where $x=0$.

Now from $(2)$, $x^2 - 4\lambda y = 0$ and substituting $x=0$ we have:

$$
\begin{aligned} 0 - 4\lambda y &= 0 \\
 0 &= 4\lambda y
\end{aligned}
$$

So either $y=0$ or $\lambda = 0$. We reject $y=0$ since substituting $x=0$ and $y=0$ into $(3)$, $x^2 +2y^2 = 6$, we get $0^2 + 0^2 = 6$ which is nonsense. So taking the case where $\lambda = 0$ and $y$ is non zero we have.
...
Now we have an extra two critical points of $(0,\sqrt{3})$ and $(0,-\sqrt{3})$.
...
$(0,\sqrt{3})$, $(2,1)$ and $(-2,1)$ are maxima.

$(0,-\sqrt{3})$, $(2,-1)$ and $(-2,-1)$ are minima.

Note that we don't know if these are local maxima and minima or global ones.

Phew! So the final answer is $(0,\sqrt{3})$, $(2,1)$ and $(-2,1)$ are the local maximum values of $x^2y$ subject to $x^2 +2y^2 = 6$. To find the absolute maximum values we would have to substitute them into the original function.
