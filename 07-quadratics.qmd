...
```{r, echo=FALSE}
knitr::include_url("https://www.desmos.com/calculator/ufd5zwqegu?embed", height="500px")
```
...
```{r, echo=FALSE}
knitr::include_url("https://numbas.mathcentre.ac.uk/question/175469/ed-s-copy-of-quadratics-factorisation-1-to-be-fixe/embed/", height="400px")
```
...
```{r, echo=FALSE}
knitr::include_url("https://numbas.mathcentre.ac.uk/question/105345/quadratics-solving-by-factorisation/embed/?token=84458480-2b4c-4902-9920-c43792afedcd", height="400px")
```
...
A firm's profit from producing $Q$ units is $\pi = -Q^2 + 8Q - 12$. The **break-even points** occur where profit is zero:
...
This factorises to $(Q-6)(Q-2)=0$. When two brackets multiply to give zero, at least one of them must be zero. So either $Q-6=0$ or $Q-2=0$. The firm breaks even when output is $Q=2$ or $Q=6$. Between these production levels the firm makes a net profit; outside them it makes a loss.

```{r, echo=FALSE, fig.height=3.5, fig.width=5, fig.align='center', fig.alt='A downward-opening parabola showing profit as a function of output. The curve crosses the horizontal axis at Q equals 2 and Q equals 6, which are the break-even points.'}
Q <- seq(0, 7, length.out = 200)
profit <- -Q^2 + 8 * Q - 12
plot(Q, profit, type = "n", xlim = c(0, 7), ylim = c(-12, 6),
     xlab = "Output, Q", ylab = expression("Profit," ~ pi),
     main = expression(pi == -Q^2 + 8*Q - 12),
     axes = FALSE)
abline(h = seq(-12, 6, by = 6), v = 0:7, col = "grey90", lty = "dotted")
axis(1, at = 0:7)
axis(2, at = seq(-12, 6, by = 6), las = 1)
abline(h = 0, v = 0, col = "grey50")
lines(Q, profit, lwd = 2)
points(c(2, 6), c(0, 0), col = "blue", pch = 19)
```
::::
...
```{r, echo=FALSE}
knitr::include_url("https://numbas.mathcentre.ac.uk/question/64345/quadratics-using-the-quadratic-formula/embed/?token=df746faf-a094-4dd6-8e2b-19ae44306fc8", height="400px")
```
...
```{r, echo=FALSE}
knitr::include_url("https://numbas.mathcentre.ac.uk/question/65155/simultaneous-equations-quadratics-1/embed/?token=582d176f-c880-4353-abdf-8f51e0c575b6", height="400px")
```
