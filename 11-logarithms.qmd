...
This turns a multiplicative relationship into a linear one, making it straightforward to estimate $\alpha$ and $\beta$ from data. For example, if output $Y$, capital $K$ and labour $L$ are all measured over time, regressing $\ln Y$ on $\ln K$ and $\ln L$ will give the coefficients $\alpha$ and $\beta$ directly.

```{r, echo=FALSE, fig.height=3.5, fig.width=5, fig.align='center', fig.alt='A graph showing a straight line representing the natural logarithm of output against the natural logarithm of capital for a Cobb-Douglas production function.'}
lnK <- seq(0, 3, length.out = 200)
lnY <- 1 + 0.5 * lnK
plot(lnK, lnY, type = "n", xlim = c(0, 3), ylim = c(0, 3),
     xlab = expression(ln*K), ylab = expression(ln*Y),
     main = expression(ln*Y == ln*A + alpha*ln*K),
     xaxp = c(0, 3, 3), yaxp = c(0, 3, 3),
     panel.first = grid())
lines(lnK, lnY, lwd = 2)
points(1, 1.5, col = "blue", pch = 19, cex = 1.3)
```
::::
